...
void MP2Node::updateRing() {
	 ...
	 */
	vector<Node> localMemList;
	bool change = false;

	/*
	 *  Step 1. Get the current membership list from Membership Protocol / MP1
	 */
	localMemList = getMembershipList();

	/*
	 * Step 2: Construct the ring
	 */
	// Sort the list based on the hashCode
	sort(localMemList.begin(), localMemList.end());


	// if #elements in localMemList != ring then remoteMemList changed
	if (localMemList.size() != ring.size())
	{
		change = true;
	}
	else {
		//localMemList and so ring are sorted, so we just compare
		//the elements at each position in the arrays
		for(int i=0; i < localMemList.size(); ++i) {
			if (localMemList[i].getHashCode() != ring[i].getHashCode()) {
				change = true;
				break;
			}
		}
...
vector<Node> MP2Node::getMembershipList() {
	...
	for ( i = 0 ; i < this->memberNode->memberList.size(); i++ ) {
		...
		localMemList.emplace_back(Node(addressOfThisMember));
	}
...
void MP2Node::checkMessages() {
	...
	while ( !memberNode->mp2q.empty() ) {
		...
		switch(msg.type) {
			 case CREATE: 
			 	onCreateMessage(msg); //I am replica
			    break;
			 case DELETE: 
			 	onDeleteMessage(msg); //I am replica
				break;
			 ...
			 case REPLY:  // I am coordinator
				...
				else if (info.msgType == DELETE) {
					onDeleteReplyMessage(msg);
				}
				else if (info.msgType == UPDATE) {
...
void MP2Node::onReadReplyMessage(Message& msg)
{
  ...
  //printTransInfo(msg.transID, info);
	...
	if (info.replyCount == 0) {
		...
		else if (!transInfo[msg.transID].value2.empty() && (transInfo[msg.transID].value2 == msg.value)) {
			log->logReadSuccess(&memberNode->addr, true, msg.transID, info.key, msg.value);
			transInfo[msg.transID].isCommited = true;
			//cout << "Quorum23" << endl;
		}
...
void MP2Node::stabilizationProtocol() {
	...
	for(auto& entry: ht->hashTable) {
		...
		else if (!(*nodes[0].getAddress() == memberNode->addr) &&
...
void MP2Node::timeouts() {
	...
	for(auto& e: transInfo) {
		...
		if ((par->getcurrtime() - info.time) > TIMEOUT) {
			...
			else if(info.msgType == READREPLY) {
